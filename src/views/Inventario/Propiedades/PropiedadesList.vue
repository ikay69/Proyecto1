<template>
  <v-container fluid class="pa-6">
    <div class="d-flex align-center justify-space-between mb-4 flex-wrap ga-2">
      <h1 class="text-h5">Propiedades</h1>
    ...
    <v-card elevation="1">
      <v-table>
        ...
        <tbody>
          ...
          <tr v-else-if="!propiedades.length">
            <td colspan="6" class="text-center py-6 text-medium-emphasis">
              No hay registros para mostrar
            </td>
          </tr>
          <tr v-for="prop in propiedades" :key="prop.proId">
            <td>{{ prop.proNombre }}</td>
            <td>{{ prop.proTipoDato }}</td>
            <td>
              <v-chip :color="prop.proEstado === 1 ? 'success' : 'error'" size="small">
                {{ prop.proEstado === 1 ? 'Activa' : 'Inactiva' }}
              </v-chip>
            </td>
            <td>{{ formatearFecha(prop.proFecCreacion) }}</td>
            <td>{{ prop.proUsuario }}</td>
            <td class="text-center">
              <v-btn icon="mdi-pencil" size="small" variant="text" @click="irAEditar(prop.proEmp, prop.proId)" />
            </td>
          </tr>
        </tbody>
      </v-table>
  ...
  methods: {
    ...
    async buscar(pagina) {
      ...
      try {
        ...
        const mensaje = error.response?.data?.msg || 'No se pudo consultar las propiedades'
    ...
    irANueva() {
      this.$router.push({ name: 'PropiedadesNueva' })
    },

    irAEditar(EmpId, PropId) {
      this.$router.push({ name: 'PropiedadesEditar', params: { EmpId, PropId } })
    },
