...
const terceroValidaDatos = async (req,res,next)=>{
    ...
    if(!vApellidos || vApellidos.length === 0){
        return res.status(401).json({msg:"Los apellidos no pueden estar vacíos"});
    }

    if(vApellidos.length > 150){
        return res.status(401).json({msg:"Los apellidos superan los 150 caracteres"});
    }
    ...
    if(vNumeroDocumento.length > 50){
        return res.status(401).json({msg:"El numero de documento superan los 50 caracteres"});
    }

    //valida letras numeros y simbolo - #, no se permite espacios
    if(!/^[a-zA-Z0-9-#]+$/.test(vNumeroDocumento)){
            return res.status(401).json({msg:"El Numero de documento solo puede contener números letras y ( - #)"});
    }
    ...
    if(NumeroDocumento !== undefined && NumeroDocumento !== null && String(NumeroDocumento).trim().length > 0){
        ...
        if(vNumeroDocumento.length > 50){
            return res.status(401).json({msg:"El numero de documento supera los 50 caracteres"});
        }
    }*/
    ...
    if(Celular !== undefined && Celular !== null && String(Celular).trim().length > 0){
        ...
        if(vCelular.length > 10){
            return res.status(401).json({msg:"El celular supera los 10 caracteres"});
        }
        if(!/^[0-9]+$/.test(vCelular)){
            return res.status(401).json({msg:"El celular solo puede contener números"});
        }
    }
    ...
    if(Email !== undefined && Email !== null && String(Email).trim().length > 0){
        ...
        if(vEmail.length > 150){
            return res.status(401).json({msg:"El email supera los 150 caracteres"});
        }
        const regexEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if(!regexEmail.test(vEmail)){
            return res.status(401).json({msg:"El email no tiene un formato válido"});
        }
    }
    ...
    if(Direccion !== undefined && Direccion !== null && String(Direccion).trim().length > 0){
        ...
        if(vDireccion.length > 200){
            return res.status(401).json({msg:"La dirección supera los 200 caracteres"});
        }
    }
