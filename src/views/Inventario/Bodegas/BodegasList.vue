    ...
    <v-card elevation="1">
      <v-table>
        ...
        <tbody>
          ...
          <tr v-else-if="!bodegas.length">
            <td colspan="5" class="text-center py-6 text-medium-emphasis">
              No hay registros para mostrar
            </td>
          </tr>
          <tr v-for="bod in bodegas" :key="bod.bodId">
            <td>{{ bod.bodNombre }}</td>
            <td>
              <v-chip :color="bod.bodEstado === 1 ? 'success' : 'error'" size="small">
                {{ bod.bodEstado === 1 ? 'Activa' : 'Inactiva' }}
              </v-chip>
            </td>
            <td>{{ formatearFecha(bod.bodFecCreacion) }}</td>
            <td>{{ bod.bodUsuario }}</td>
            <td class="text-center">
              <v-btn icon="mdi-pencil" size="small" variant="text" @click="irAEditar(bod.bodEmp, bod.bodId)" />
            </td>
          </tr>
        </tbody>
      </v-table>
  ...
  methods: {
    ...
    async buscar(pagina) {
      ...
      try {
        ...
        const mensaje = error.response?.data?.msg || 'No se pudo consultar las bodegas'
    ...
    irANueva() {
      this.$router.push({ name: 'BodegasNueva' })
    },

    irAEditar(EmpId, BodId) {
      this.$router.push({ name: 'BodegasEditar', params: { EmpId, BodId } })
    },
