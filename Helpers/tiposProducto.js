...
const tipoProdValidaNombre = async (req,res,next)=>{
    ...
    if(vNombre.length > 100){
        return res.status(401).json({msg:"El nombre supera los 300 caracteres"});
    }

    const regex = /^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s#-]+$/;
    if (!regex.test(vNombre)){
        return res.status(401).json({msg:"El nombre solo debe contener letras y números, con espacios y los simbolos # - "});
    }
...
const tipoProdValidaFiltros = async (req,res,next)=>{
    ...
    if(textoFiltro === undefined || !textoFiltro || textoFiltro.trim().length === 0){
        ...
        if(vTextoFiltro.length>300){
            return res.status(401).json({msg:'Texto de filtro supera los 300 caracteres'});
        }
    }
