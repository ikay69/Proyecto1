<template>
  <v-container fluid class="pa-6">
    <div class="d-flex align-center justify-space-between mb-4 flex-wrap ga-2">
      <h1 class="text-h5">Unidades de medida</h1>
    ...
    <v-card elevation="1">
      <v-table>
        ...
        <tbody>
          ...
          <tr v-else-if="!unidadesMedida.length">
            <td colspan="6" class="text-center py-6 text-medium-emphasis">
              No hay registros para mostrar
            </td>
          </tr>
          <tr v-for="uni in unidadesMedida" :key="uni.uniMedId">
            <td>{{ uni.uniMedNombre }}</td>
            <td>{{ uni.uniMedSimbolo }}</td>
            <td>
              <v-chip :color="uni.uniMedEstado === 1 ? 'success' : 'error'" size="small">
                {{ uni.uniMedEstado === 1 ? 'Activa' : 'Inactiva' }}
              </v-chip>
            </td>
            <td>{{ formatearFecha(uni.uniMedFecCreacion) }}</td>
            <td>{{ uni.uniMedUsuario }}</td>
            <td class="text-center">
              <v-btn icon="mdi-pencil" size="small" variant="text" @click="irAEditar(uni.uniMedEmp, uni.uniMedId)" />
            </td>
          </tr>
        </tbody>
      </v-table>
  ...
  methods: {
    ...
    async buscar(pagina) {
      ...
      try {
        ...
        const mensaje = error.response?.data?.msg || 'No se pudo consultar las unidades de medida'
    ...
    irANueva() {
      this.$router.push({ name: 'UnidadesMedidaNueva' })
    },

    irAEditar(EmpId, UniMedId) {
      this.$router.push({ name: 'UnidadesMedidaEditar', params: { EmpId, UniMedId } })
    },
