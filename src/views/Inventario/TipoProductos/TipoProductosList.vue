<template>
  <v-container fluid class="pa-6">
    <div class="d-flex align-center justify-space-between mb-4 flex-wrap ga-2">
      <h1 class="text-h5">Tipos de producto</h1>
    ...
    <v-card elevation="1">
      <v-table>
        ...
        <tbody>
          ...
          <tr v-else-if="!tiposProducto.length">
            <td colspan="5" class="text-center py-6 text-medium-emphasis">
              No hay registros para mostrar
            </td>
          </tr>
          <tr v-for="tp in tiposProducto" :key="tp.tipProId">
            <td>{{ tp.tipProNombre }}</td>
            <td>
              <v-chip :color="tp.tipProEstado === 1 ? 'success' : 'error'" size="small">
                {{ tp.tipProEstado === 1 ? 'Activo' : 'Inactivo' }}
              </v-chip>
            </td>
            <td>{{ formatearFecha(tp.tipProFecCreacion) }}</td>
            <td>{{ tp.tipProUsuario }}</td>
            <td class="text-center">
              <v-btn icon="mdi-pencil" size="small" variant="text" @click="irAEditar(tp.tipProEmp, tp.tipProId)" />
            </td>
          </tr>
        </tbody>
      </v-table>
  ...
  methods: {
    ...
    async buscar(pagina) {
      ...
      try {
        ...
        this.tiposProducto = data.data || []
        this.cantData = data.cantData || 0
        this.pagina = pagina
      } catch (error) {
        const mensaje = error.response?.data?.msg || 'No se pudo consultar los tipos de producto'
    ...
    irANuevo() {
      this.$router.push({ name: 'TipoProductosNuevo' })
    },

    irAEditar(EmpId, TipProId) {
      this.$router.push({ name: 'TipoProductosEditar', params: { EmpId, TipProId } })
    },
