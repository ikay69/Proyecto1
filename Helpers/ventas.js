...
const ventaValidaDatos = async (req,res,next) => {
    ...
    if (!TIPOS_VENTA_VALIDOS.includes(TipoVenta)) {
        return res.status(400).json({msg:'Tipo de venta inválido'});
    }
    ...
    if (Articulos.length > 200) {
        return res.status(400).json({msg:'La venta no puede tener más de 200 líneas'});
    }
    for (const item of Articulos) {
        //sin este guarda, un elemento null de la lista haria estallar el acceso a
        //item.idArticulo y la ruta respondería 500 (con stack trace) en vez de un 400
        ...
        if (isNaN(Number(item.PrecioVentaUnidad)) || Number(item.PrecioVentaUnidad) <= 0) {
            return res.status(400).json({msg:'PrecioVentaUnidad inválido'});
        }
    }
