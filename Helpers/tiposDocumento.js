...
const tipoDocValidaDatos = async (req,res,next)=>{
    ...
    if (vAbreviatura === undefined || !vAbreviatura || vAbreviatura.length === 0){
        return res.status(401).json({msg:"La abreviatura no puede estar vacía"});
    }

    if(vAbreviatura.length > 10){
        return res.status(401).json({msg:"La abreviatura supera los 10 caracteres"});
    }

    if (vDescripcion === undefined || !vDescripcion || vDescripcion.length === 0){
        return res.status(401).json({msg:"La descripción no puede estar vacía"});
    }

    if(vDescripcion.length > 100){
        return res.status(401).json({msg:"La descripción supera los 100 caracteres"});
    }
