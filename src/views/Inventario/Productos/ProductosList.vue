      ...
      <v-table>
        ...
        <tbody>
          ...
          <tr v-for="prod in productos" :key="prod.proId">
            ...
            <td>
              <v-chip :color="prod.proEstado === 1 ? 'success' : 'error'" size="small">
                {{ prod.proEstado === 1 ? 'Activo' : 'Inactivo' }}
              </v-chip>
            </td>
            <td>{{ formatearFecha(prod.proFecCreacion) }}</td>
            <td class="text-center">
              <v-btn icon="mdi-pencil" size="small" variant="text" @click="irAEditar(prod.proEmp, prod.proId)" />
            </td>
          </tr>
        </tbody>
      </v-table>
...
export default {
  ...
  methods: {
    ...
    irAEditar(EmpId, ProdId) {
      this.$router.push({ name: 'ProductosEditar', params: { EmpId, ProdId } })
    },
